use glium::*;
use nalgebra::{Matrix4, Point3, Similarity3, Vector3};
...
fn point(bits: u32, dim: u32, size: f32) -> Vec<f32> {
    ...
    for i in 0..dim {
        let b = if ((1 << i) & bits) == 0 { -size } else { size };
        v[i as usize] = b;
    }
    v
}
...
impl Cube {
    ...
    pub fn intersections(
        ...
        let mut isects: Vec<(Point3<f32>, Face)> = self
            .faces
            .iter()
            .filter_map(|face| {
                face.intersect(origin, dir)
                    .map(|isect| (isect, face.clone()))
            })
            .collect();
        isects.sort_by_key(|(isect, _)| {
            NotNan::new((origin - isect).norm()).expect("Distance should not be NaN")
        });
        isects
    }

    // FIXME: Should the cube own it's shader??
    pub fn render<S: Surface>(&self, view_proj: Matrix4<f32>, shader: &Program, target: &mut S) {
